...
def getNormalizer(_user_N,_pro_N,_gf_measure):
    ...
    if current_normalizer_key in normalizer_dic.keys():
        ...
        #appendNormalizerDictionary(_user_N, _pro_N, _gf_measure, float(normalizer))           
...
def appendNormalizerDictionary(_user_N,_pro_N,_gf_measure,_normalizer):
    """
    Add the values as a line to the normalizer external txt
    """
    try:
        with open(NORM_FILE, "a") as f:
            f.write(f"{_user_N},{_pro_N},{_gf_measure}:{_normalizer}\n")
    except Exception as e:
        print("Error writing to normalizer file")
